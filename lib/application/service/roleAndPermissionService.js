...
class Service {
    ...
    registerPermission(permissionData, traceContext, callback) {
        ...
        function* registerPermission() {
            ...
            if (permission) {
                logger.warn("this permission exist", traceContext);
                return false;
            }
            let newPermission = {};
    ...
    getPermissions(traceContext, callback) {
        ...
        self._permissionRepository.getAllPermission(traceContext, (err, result) => {
            if (err) {
                logger.error("this permissions is no exist", traceContext);
                callback(err, null);
                return;
            }
            if (!result) {
    ...
    getPermission(permissionID, traceContext, callback) {
        ...
        self._permissionRepository.getPermissionByID(permissionID, traceContext, (err, permission) => {
            if (err) {
                logger.error("this permission is no exist", traceContext);
                callback(err, null);
                return;
            }
            if (!permission) {
    ...
    registerRole(roleData, traceContext, callback) {
        ...
        function* registerRole() {
            ...
            if (role) {
                logger.warn("this role exist", traceContext);
                return false;
            }
            let newRole = {};
            ...
            newRole.permissionIDs.push(roleData.permissionID);
    ...
    assignPermissionToRole(permissionID, roleID, traceContext, callback) {
        ...
        function* assignPermission() {
            ...
            let role = yield getRoleFromRepository();
            if (!role) {
                logger.error("this role is no exist", traceContext);
                return false;
            }
            let result = role.addPermission(permission.permissionID);
            if(!result){
                logger.warn("This role has this permission", traceContext);
                return false;
            }
    ...
    canclePermissionofRole(permissionID, roleID, traceContext, callback) {
        ...
        function* assignPermission() {
            ...
            let result = role.removePermission(permission.permissionID);
            if(!result){
                logger.warn("The role does not have this permission", traceContext);
                return false;
            }
            let isSuccess = yield saveRole(role);
            return isSuccess;
