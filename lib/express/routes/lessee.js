...
router.post('/lessees/:lesseeID/stations', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.addStationToLessee(lesseeID, stationData, traceContext, (err, stationID)=> {console.log(stationData);
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/lessees', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.getLessees(lesseeID, traceContext, (err, lessees)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/stations', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.getStations(stationID, traceContext, (err, stations)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.post('/dataSources', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.registerDataSource(dataSource, traceContext, (err, isSuccess)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.delete('/dataSources/:dataSourceID', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.delDataSource(dataSourceID, traceContext, (err, isSuccess)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/dataSources', (req, res)=> {
    ...
    let lesseeService = req.app.get('lesseeService');
    let traceContext = traceContextFeach(req);
    lesseeService.getDataSources(dataSourceID, traceContext, (err, datas)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
