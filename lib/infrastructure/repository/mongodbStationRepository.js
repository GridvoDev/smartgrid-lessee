...
class Repository {
    ...
    getStationByID(stationID, traceContext, callback) {
        let mongoClient = createMongoZipkinClient({
            tracer,
            traceContext,
            dbName: this._dbName,
        ...
        mongoClient.then(({db, collection}) => {
            collection.findOne({stationID}, {limit: 1}, (err, document) => {
                if (err) {
                    callback(err, null);
                    db.close();
                    return;
            ...
            collection.aggregate([],
                (err, result) => {
                    ...
                    for (let document of result) {
                        let {stationID, stationInfo:{stationName}, lesseeID, members} = document;
                        stations.push({stationID, stationInfo: {stationName}, lesseeID, members});
                    }
                    callback(null, stations);
                    db.close();
                });
