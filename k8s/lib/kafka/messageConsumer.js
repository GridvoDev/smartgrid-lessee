...
class Consumer {
    ...
    startConsume() {
        ...
        let self = this;
        this._consumer.consumeMessage(topics, (err, message)=> {console.log(message);
            if (err) {
            ...
            switch (message.topic) {
                case "corp-auth-smartgrid-suite":
                    self._lesseeService.changeCorpWechatActiveState(data.corpID, true, traceContext, (err, isSuccess)=> {
                        ...
                        if (isSuccess) {
                            logger.info(`corp ${data.corpID} wechat state activate success`, traceContext);
                        } else {
                            logger.error(`corp ${data.corpID} wechat state activate fail`, traceContext);
                        }
                    });
                    return;
                case "corp-cancel-auth-smartgrid-suite":
                    self._lesseeService.changeCorpWechatActiveState(data.corpID, false, traceContext, (err, isSuccess)=> {
                        ...
                        if (isSuccess) {
                            logger.info(`corp ${data.corpID} wechat state cancel activate success`, traceContext);
                        } else {
                            logger.error(`corp ${data.corpID} wechat state cancel activate fail`, traceContext);
                        }
                    });
                    return;
                default:
